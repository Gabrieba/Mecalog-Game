...
// Also compute robot position if it's moving while the block is falling
void gravityBlocks(tabsprite* tabBlocs, tabsprite* tabScenery, tabplat* tabButtons, char** inputKey, int shifting) {
  ...
  while (motionless == 0) {
    ...
    for (i = 0; i < tabBlocs->number; i++) {
      ...
      if (bit != 1) {     // If no conflict between two blocs
      ...
      if (bit == 0) {       // If no conflict between two blocs or between a bloc and a scenaristic element
        (tabBlocs->tab)[i].position.y += shifting;
        motionless = 0;
      }
    }
    if (motionless != 1) {      // If a block is still moving
      SDL_Delay(time[24-k]);
      inputEvent(inputKey);
    ...
    if (  (conflictY((tabScenery->tab)[2], (tabBlocs->tab)[i]) == 1) &&
         ...
         for (j = 0; j < tabBlocs->number; j++) {
           ...
           if (j != i && conflictX((tabBlocs->tab)[i], (tabBlocs->tab)[j]) && conflictY((tabBlocs->tab)[i], (tabBlocs->tab)[j])) {
             ...
             (tabScenery->tab)[2].position.x = (tabBlocs->tab)[i].position.x + (tabBlocs->tab)[i].dimx;
             gravityBlocks(tabBlocs, tabScenery, tabButtons, inputKey, 4);
             return;
           }
         }
